...
bpoint := IntToHex(ReadMemory(cm.processPatternScan(,, cm.hexStringToPattern(ptrScan)*)+1, pidstart) - Base)
rpoint := IntToHex(ReadMemory(cm.processPatternScan(,, cm.hexStringToPattern(rptrScan)*)+1, pidstart) - Base)
fpoint := IntToHex(ReadMemory(cm.processPatternScan(,, cm.hexStringToPattern(fishptrScan)*)+6, pidstart) - Base)

readPointer(){
    return bpoint
}
IniWrite, %bpoint%, config.ini, Pointers, base
